 ...
 * Valida obligatorios, rangos y estados; normaliza entradas y mapea DTO.
 */
@ApplicationScoped
public class VehiculoService {

    private static final Set<String> ESTADOS_VALIDOS = Set.of("disponible", "rentado", "mantenimiento");
    ...
    public List<Vehiculo> listarPorCategoriaDesc(String descripcion) {
        if (descripcion == null) return List.of();
        return repository.findByCategoriaDescripcion(descripcion.trim());
    }
    ...
    public void addVehiculo(VehiculoDto dto) {
        try {
            ...
            normalize(dto);
            // Si quieres evitar placas duplicadas aquí:
            // if (repository.existsByPlaca(dto.getPlaca())) throw new IllegalArgumentException("La placa ya existe: " + dto.getPlaca());
            repository.addVehiculo(dto);
        } catch (Exception e) {
            throw new RuntimeException("No se puede agregar el vehículo", e);
        }
    }
    ...
    public Optional<VehiculoDto> buscarPorPlaca(String placa) {
        ...
        return Optional.ofNullable(repository.findVehiculoByPlaca(placa.trim())).map(this::toDto);
    }

    public Optional<Vehiculo> buscarEntidadPorPlaca(String placa) {
        if (placa == null || placa.isBlank()) return Optional.empty();
        try { return Optional.ofNullable(repository.findVehiculoByPlaca(placa.trim())); }
        catch (Exception e) { return Optional.empty(); }
    }

    public boolean existePlaca(String placa) { return placa != null && repository.existsByPlaca(placa.trim()); }
    ...
    private void validarObligatorios(VehiculoDto v) {
        ...
        if (isBlank(v.getPlaca())) throw new IllegalArgumentException("La placa es obligatoria.");
        if (isBlank(v.getModelo())) throw new IllegalArgumentException("El modelo es obligatorio.");
        if (isBlank(v.getMarca()))  throw new IllegalArgumentException("La marca es obligatoria.");
        ...
        } else if (!ESTADOS_VALIDOS.contains(v.getEstado().trim().toLowerCase())) {
        ...
        if (v.getAnio() < 1900 || v.getAnio() > max)
            throw new IllegalArgumentException("El año debe estar entre 1900 y " + max + ".");
    ...
    private void normalize(VehiculoDto v) {
        if (v.getPlaca() != null)  v.setPlaca(v.getPlaca().trim());
        if (v.getModelo() != null) v.setModelo(v.getModelo().trim());
        if (v.getMarca() != null)  v.setMarca(v.getMarca().trim());
        if (v.getEstado() != null) v.setEstado(v.getEstado().trim().toLowerCase());
        // categoría: nada que normalizar aquí
    }
    ...
    public List<VehiculoDto> listarPorEstado(String estado) {
        if (estado == null || estado.isBlank()) { return listar(); }
        String normalized = estado.trim().toLowerCase();
        if (!ESTADOS_VALIDOS.contains(normalized)) {
            throw new IllegalArgumentException("Estado inválido. Solo se permiten: " + ESTADOS_VALIDOS);
        }
        return repository.findByEstado(normalized).stream().map(this::toDto).toList();
    }
